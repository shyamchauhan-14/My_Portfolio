...
class BankDB:
    ...
    def create_loan(self, account_id, principal, interest_rate, months):
        acc = self.get_account(account_id)
        if not acc:
            raise ValueError("Account not found")
        ...
        cur.execute("""
            INSERT INTO loans (
            account_id, principal, interest_rate,
            total_amount, emi, months,
            start_date, status
        )
        VALUES (?,?,?,?,?,?,?,?)
    """, (account_id, principal, interest_rate,
          total, emi, months, ts, "ACTIVE"))
        self.conn.commit()
    ...
    def pay_loan(self, loan_id, amount):
        cur = self.conn.cursor()
        cur.execute(
            "SELECT total_amount, paid_amount FROM loans WHERE id=?",
            (loan_id,)
    )
        loan = cur.fetchone()
        if not loan:
        ...
        if paid + amount > total:
            raise ValueError("Payment exceeds remaining loan amount")

        new_paid = paid + amount
        status = "PAID" if new_paid == total else "ACTIVE"
...
class BankApp(tk.Tk):
    def __init__(self):
        ...
        heading=tk.Label(text="Shyam Capital & Finance",font=("Times New Roman","22","bold"),bg="#001F3F",fg="white",padx=610)
        heading.pack()
        ...
        self.notebook.add(self.loan_tab, text="Loan")


        style=ttk.Style()
        style.theme_use("default")
        style.configure("TNotebook.Tab",background="#FFD700",foreground="black")
    ...
    def _setup_accounts_tab(self):
        ...
        self.configure(bg=button_color)

        style = ttk.Style()
        style.configure("Maroon.TButton",
        ...
        ttk.Button(create_frame, text="Create",
               ...
               style="Maroon.TButton").grid(row=0, column=4, padx=6, pady=6)
    ...
    def _setup_transactions_tab(self):
        ...
        columns = ("id", "type", "amount", "timestamp", "note", "related","loan")
    ...
    def _setup_loan_tab(self):
        ...
        loan_box = ttk.LabelFrame(frm, text="Create Loan")
        loan_box.pack(fill=tk.X, padx=8, pady=8)

        self.loan_acc_var = tk.StringVar()
        self.loan_amt_var = tk.StringVar()
        self.loan_rate_var = tk.StringVar()
        self.loan_months_var = tk.StringVar()


        ttk.Label(loan_box, text="Account ID").grid(row=0, column=0)
        ttk.Entry(loan_box, textvariable=self.loan_acc_var).grid(row=0, column=1)

        ttk.Label(loan_box, text="Amount").grid(row=0, column=2)
        ttk.Entry(loan_box, textvariable=self.loan_amt_var).grid(row=0, column=3)

        ttk.Label(loan_box, text="Interest %").grid(row=0, column=4)
        ttk.Entry(loan_box, textvariable=self.loan_rate_var).grid(row=0, column=5)

        ttk.Label(loan_box, text="Months").grid(row=0, column=6)
        ttk.Entry(loan_box, textvariable=self.loan_months_var).grid(row=0, column=7)


        ttk.Button(loan_box, text="Create Loan",
               command=self.create_loan).grid(row=0, column=8)

        list_box = ttk.LabelFrame(frm, text="Loans")
        list_box.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)

        columns = ("id","account","principal","rate","total","emi","months","paid","status")
        self.loan_tree = ttk.Treeview(list_box, columns=columns, show="headings")
        for c in columns:
            self.loan_tree.heading(c, text=c.upper())
            self.loan_tree.column(c, anchor=tk.CENTER)
        self.loan_tree.pack(fill=tk.BOTH, expand=True)

        pay_box = ttk.LabelFrame(frm, text="Pay Loan")
        pay_box.pack(fill=tk.X, padx=8, pady=8)

        self.pay_loan_id = tk.StringVar()
        self.pay_amt_var = tk.StringVar()

        ttk.Label(pay_box, text="Loan ID").grid(row=0, column=0)
        ttk.Entry(pay_box, textvariable=self.pay_loan_id).grid(row=0, column=1)

        ttk.Label(pay_box, text="Amount").grid(row=0, column=2)
        ttk.Entry(pay_box, textvariable=self.pay_amt_var).grid(row=0, column=3)

        ttk.Button(pay_box, text="Pay",
               command=self.pay_loan).grid(row=0, column=4)

        self.load_loans()

    def create_loan(self):
        try:
            acc = int(self.loan_acc_var.get())
            amt = float(self.loan_amt_var.get())
            rate = float(self.loan_rate_var.get())
            months = int(self.loan_months_var.get())

            self.db.create_loan(acc, amt, rate, months)
            messagebox.showinfo("Success", "Loan created with EMI")
            self.load_loans()
        except ValueError as e:
            messagebox.showerror("Error", str(e))
    ...
    def pay_loan(self):
        try:
            loan_id = int(self.pay_loan_id.get())
            amt = float(self.pay_amt_var.get())
            self.db.pay_loan(loan_id, amt)
            messagebox.showinfo("Success", "Loan payment successful")
            self.load_loans()
        except ValueError as e:
            messagebox.showerror("Error", str(e))
    ...
    def load_loans(self):
        ...
        for loan in self.db.list_loans():
            self.loan_tree.insert("", tk.END, values=loan)
